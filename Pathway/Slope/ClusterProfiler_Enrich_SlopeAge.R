...
# Load data
SlopeResults <- read.csv("./Anova_output/Condensed_kidney_anova_output_slope_pvalONLYsig.csv")
...
for(i in 1:length(QuadList)){
  ...
  write.csv(go_result, paste0("./Pathways/AgeSlope_EnrichGObp_",type,".csv"), row.names=FALSE)

  go_result <- go_cc@result
  write.csv(go_result, paste0("./Pathways/AgeSlope_EnrichGOcc_",type,".csv"), row.names=FALSE)

  write.csv(kegg_result, paste0("./Pathways/AgeSlope_EnrichKEGG_",type,".csv"), row.names=FALSE)
}
