...
anova_tests_2 <- function(x) {
  ...
  pvalues <- c(subset(lm.full, term=="Age12")$p.value,
               subset(lm.full, term=="Age18")$p.value,
  ...
  tmp.age  <- lm(as.numeric(as.character.factor(Age)) ~ Sex + Generation, data=annot.samples[pres,])
