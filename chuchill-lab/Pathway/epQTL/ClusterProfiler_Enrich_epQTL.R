...
for( type in c("mrna", "protein")){
  ...
  if(type == "mrna"){
    ...
    ScanResult <- read.csv("./results/QTLscan/mRNA/BestMarker_AgeInt_mrna.csv", stringsAsFactors = FALSE)
    thr <- 7
    chr <- "12"
    ...
    # get data and specify query
    ScanResult <- read.csv("./results/QTLscan/protein/BestMarker_AgeInt_protein.csv", stringsAsFactors = FALSE)
    thr <- 8
    chr <- "7"
