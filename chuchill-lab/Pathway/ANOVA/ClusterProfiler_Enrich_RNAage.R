...
# Load data
load("./data/Rdata/DO188b_kidney.RData")
rm(genoprobs, G, Glist, N, raw.protein, snps)
AnovResults <- read.csv("./results/ANOVA/mrna.kidney_anova_table.csv")

# subset for only significant genes < 0.05
sig <- AnovResults[AnovResults$sig.mRNA_Age.Sex == TRUE,]
...
#  16.28% of input gene IDs are fail to map...
...
write.csv(go_result, "./results/Pathways/RNA_EnrichGObp_Age.csv", row.names = FALSE)
# GO: CC
...
write.csv(go_result, "./results/Pathways/RNA_EnrichGOcc_Age.csv", row.names = FALSE)
...
write.csv(kegg_result, "./results/Pathways/RNA_EnrichKEGG_Age.csv", row.names = FALSE)
