...
AnovResults <- read.csv("./results/ANOVA/protein.kidney_anova_table.csv")

# subset for only significant genes < 0.05
sig <- AnovResults[AnovResults$sig.Prot_Sex.Age == TRUE,]
...
write.csv(go_result, "./results/Pathways/PROT_EnrichGObp_Sex.csv", row.names = FALSE)
...
write.csv(go_result, "./results/Pathways/PROT_EnrichGOcc_Sex.csv", row.names = FALSE)
...
write.csv(kegg_result, "./results/Pathways/PROT_EnrichKEGG_Sex.csv", row.names = FALSE)
