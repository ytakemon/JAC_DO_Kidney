...
# subset data
sub_annot.protein <- annot.protein %>% filter(id %in% Chr7_list$id) %>% arrange(id) #dim(sub_annot.protein)
...
annot.samples$Med <- expr.protein[,annot.protein[annot.protein$symbol == "Mapk3",]$id]
...
filename <- paste0("./QTLscan/scanBestMarker_protein/maxLODscan_Erk1_p/maxLODscan_batch_",plist[1],".csv")
