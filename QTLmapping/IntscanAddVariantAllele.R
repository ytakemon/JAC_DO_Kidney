...
for (i in 1:nrow(pQTL)){
  ...
  max.marker <- rownames(fit.chr)[which.max(fit.chr$pheno1)]
  if (level=="mRNA") ens <- other.ids(symbol, level)[[1]] else ens <- other.ids(symbol, level)[[3]]
  # Skip to next iteration if protein doesn't exist.
  if (is.na(ens)){
    next
  }
  if (level=="mRNA") y <- expr.mrna[,ens] else y <- expr.protein[,ens]
  ...
  dt_max <- dt[which(abs(dt$beta) == max(abs(dt$beta))),]
  pQTL$AlleleVariant[i] <- as.character(dt_max$Allele[1])
  pQTL$AgeVariant[i] <- as.character(dt_max$Age[1])
}
write.csv(pQTL, file = "/projects/korstanje-lab/ytakemon/JAC_DO_Kidney/QTLscan/output/Threshold8_pQTL_intAge_annotated.csv")
