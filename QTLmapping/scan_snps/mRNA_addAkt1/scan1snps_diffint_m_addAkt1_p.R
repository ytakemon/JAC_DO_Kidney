...
output <- sub_annot.mrna[plist,-ncol(sub_annot.mrna)]
output$AgeIntLOD  <- output$AgeIntPos <- output$AgeIntChr <- NA

# add Akt1 mRNA to annot.samples
annot.samples$Med <- expr.protein[,annot.protein[annot.protein$symbol == "Akt1",]$id]
...
write.csv(output, file = paste0("./SNPscan/diffscansnp_mrna_addAkt1_p/maxLODscan_batch_",plist[1],".csv"),row.names = FALSE)
print(Sys.time())

# Following warning messages will appear and its fine:
#Warning messages:
#1: In scan1snps(genoprobs = probs, map = map, kinship = Glist, pheno = sub_expr.mrna[,  :
