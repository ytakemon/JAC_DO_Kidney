...
# parameters
diffscan_dir <- "./SNPscan/diffscansnp_prot_addErk1_m"
IntAge_output_file <- "./SNPscan/scan1snps_p_diffAgeInt_addErk1_m_BestperGene.csv"
...
gatherdf <- function(dir, pattern){
  ...
  # return df
  output <- output %>% rename(gene_id = id) %>% mutate(
    id = annot.protein[annot.protein$gene_id %in% gene_id,]$id
  ) %>% arrange(id) %>% select(id, gene_id, symbol, chr, start, end, strand, middle_point, nearest_snp, biotype, duplicated, AgeIntChr, AgeIntPos, AgeIntLOD)
  return(output)
}
...
compare <- initialLOD %>% mutate(
  Erk1_m_MedChr = medLOD$AgeIntChr,
  Erk1_m_MedPos = medLOD$AgeIntPos,
  Erk1_m_MedLOD = medLOD$AgeIntLOD,
  LODdrop = AgeIntLOD - Erk1_m_MedLOD
)

# write table
write_csv(compare, "./SNPscan/scan1snps_p_Erk1_m_mediation_LODcomapre.csv")

# plot lod scores
pdf("./SNPscan/scan1snps_p_Erk1_m_mediation_LODcompare.pdf", width = 9, height = 9)
ggplot(compare, aes(x=AgeIntLOD, y =Erk1_m_MedLOD))+
