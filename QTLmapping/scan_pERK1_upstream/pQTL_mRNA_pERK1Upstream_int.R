# run with:
# qsub -v script=pQTL_mRNA_pERK1Upstream_int Rsubmit_args.sh
library(qtl2)
library(qtl2geno)
library(qtl2scan)
library(qtl2convert)
...
for (g in 1:nrow(E_query_list)){
  ...
  for (p in 1:length(list)){
    ...
    # Save lod object
    file_name <- paste0("./QTLscan/intscan_mrna_pERK1Upstream/Add_", med_query,"_Medscan_", p$id, "_", p$symbol, ".rds")
