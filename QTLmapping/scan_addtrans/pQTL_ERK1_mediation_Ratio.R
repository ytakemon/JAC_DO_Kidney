# qsub -v script=pQTL_ERK1_mediation_Ratio Rsubmit_args.sh
...
gene <- "Phosphrylated/ Total ERK1 ratio"
g <- "ERK1Ratio"
...
pdf(paste0("./QTLscan/output/plots/pQTL_", g, "_Mediation_chr7_thr6.pdf"), width = 9, heigh =9)
ggplot(compare, aes(x=IntAgeLODDiff,  y=addIntAgeLODDiff)) +
  ...
  scale_y_continuous( name = paste0("LOD score (X | ", gene," mRNA)"),
                      breaks = seq(0, 15, by = 1),
                      labels = seq(0, 15, by = 1)) +
  theme_bw() +
