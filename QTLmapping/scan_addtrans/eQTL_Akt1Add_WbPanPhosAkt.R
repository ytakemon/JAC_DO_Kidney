# run with:
# qsub -v script=eQTL_Akt1Add_WbPanPhosAkt2 Rsubmit_args.sh
# qsub -v script=eQTL_Akt1Add_WbPanPhosAkt4 Rsubmit_args.sh
# qsub -v script=eQTL_Akt1Add_WbPanPhosAkt6 Rsubmit_args.sh
# qsub -v script=eQTL_Akt1Add_WbPanPhosAkt8 Rsubmit_args.sh
# qsub -v script=eQTL_Akt1Add_WbPanPhosAkt9 Rsubmit_args.sh
...
akt <- akt[complete.cases(akt$Phospho_Pan.AKT),]

# cleanup Akt data
akt$duplicated <- (duplicated(akt$ID) | duplicated(akt$ID, fromLast = TRUE))
akt <- akt[akt$duplicated == FALSE,]
akt <- akt[akt$ID %in% rownames(genoprobs),] # 55 animals (33 females, 22 males)
rownames(akt) <- akt$ID
