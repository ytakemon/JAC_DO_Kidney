# run with:
# qsub -v script=pQTL_PdgfbAdd Rsubmit_args.sh
...
# Identify gene name
# PDGFB was not detected in protein data, but RNA expression data is available
...
for (p in 1:length(list)){
  ...
  # Save lod object
  file_name <- paste0("./QTLscan/addscan_prot_Pdgfb/", p$protein_id, "_", p$symbol, ".rds")
