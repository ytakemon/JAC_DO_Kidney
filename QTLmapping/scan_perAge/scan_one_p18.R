...
#subset to age group
genoprobs <- genoprobs[annot.samples$Age == "18",,]
expr.protein <- expr.protein[annot.samples$Age == "18",]
annot.samples <- annot.samples[annot.samples$Age == "18",]
...
for (p in plist) {
  ...
  # save lod object
  file_name <- paste0("./QTLscan/addscan_prot_perAge/18mo/", annot.protein$id[p], "_", annot.protein$symbol[p], ".rds")
