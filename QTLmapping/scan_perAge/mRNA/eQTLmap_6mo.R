...
eQTL_best <- read_csv("./QTLscan/output/6mo_eQTLBestperGene.csv")
...
eQTL <- ggplot(AddQTL, aes(x= q_gbm, y= t_gbm, colour = AdditiveLOD)) +
      ...
      labs( title = paste0("6 month eQTLs, threshold @",thr)) +
      theme_bw() +
...
pdf(paste0("./QTLscan/output/plots/6mo_eQTL_thr",thr,"_density.pdf"), width = 9, heigh =10)
