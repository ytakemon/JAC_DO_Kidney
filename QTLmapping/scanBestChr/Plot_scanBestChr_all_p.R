...
best <- read.csv("./QTLscan/scanBestChr_protein/BestChr_BestperGene_protein.csv")

# check distribution of LOD scores
hist(best$IntAgeMaxLODDiff, breaks =100)
# picking blunt threshold
LODthreshold_diff <- 8.6
...
best$IntAgeMaxChr <- factor(best$IntAgeMaxChr, levels= chr_full)
...
Int_age <- best[(best$IntAgeMaxLODDiff > LODthreshold_diff),] # above diff threshold

# Annotate Interactive-Age postion with genes and save file for sharing
save_int_age <- arrange(Int_age, IntAgeMaxChr, IntAgeMaxPos)
write_csv(save_int_age, path = paste0("./QTLscan/scanBestChr_protein/BestChr_BestperGene_protein_thr",LODthreshold_diff,".csv"))
...
q.gmb <- Int_age$IntAgeMaxPos # qtl
...
for(i in c(2:19, "X")) {
  ...
  wh <- which(Int_age$IntAgeMaxChr == i)
...
# eQTL plot
Plot <- ggplot(Int_age, aes(x= q_gbm, y= t_gbm, color = IntAgeMaxLODDiff)) +
      ...
      labs( title = "Interactive-Age protein QTLscan by Chr") +
      theme_bw() +
      ...
      scale_colour_gradient2(low = "blue", high = "red", mid = "blue", midpoint = mean(Int_age$IntAgeMaxLODDiff))
...
for ( i in interval){
  ...
  if(nrow(df) == 0){
    ...
    avgLOD <- c(avgLOD, mean(df$IntAgeMaxLODDiff))
  }
}
...
pdf(paste0("./QTLscan/scanBestChr_protein/BestChr_BestperGene_protein_thr",LODthreshold_diff,".pdf"), width = 9, heigh =10)
pushViewport(viewport( layout = grid.layout(10,10)))
print(Plot, vp = viewport(layout.pos.row = 1:8, layout.pos.col = 1:10))
