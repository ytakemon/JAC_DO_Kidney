...
m_best <- read.csv("./QTLscan/scanBestChr_mrna/BestChr_BestperGene_mrna.csv")
...
m_best$IntAgeMaxChr <- factor(m_best$IntAgeMaxChr, levels= chr_full)
...
Int_age <- m_best[(m_best$IntAgeMaxLODDiff > LODthreshold_diff),] # above diff threshold
...
write_csv(save_int_age, path = paste0("./QTLscan/scanBestChr_mrna/BestChr_BestperGene_mrna_thr",LODthreshold_diff,".csv"))
...
# eQTL plot
mPlot <- ggplot(Int_age, aes(x= q_gbm, y= t_gbm, color = IntAgeMaxLODDiff)) +
      ...
      labs( title = "Interactive-Age mRNA QTLscan by Chr") +
      theme_bw() +
...
pdf(paste0("./QTLscan/scanBestChr_mrna/BestChr_BestperGene_mrna_thr",LODthreshold_diff,".pdf"), width = 9, heigh =10)
