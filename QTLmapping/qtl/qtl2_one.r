...
for (i in from_i:to_i) {
  ...
  tmp <- scan1(genoprobs=get(table_of_scans$genoprobs[i]),
               ...
               addcovar=get(table_of_scans$addcovar[i]), cores=1, reml=FALSE)
  lods[[i]] <- tmp$lod
  
  tmp_coefs <- NULL
  for (c in chrs) {
    tmp2 <- scan1coef(genoprobs = get(table_of_scans$genoprobs[i])[,c], 
                      pheno = get(table_of_scans$pheno[i])[, table_of_scans$col[i]], 
                      addcovar = get(table_of_scans$addcovar[i]), 
                      kinship=get(table_of_scans$kinship[i])[[c]], reml=FALSE)
    tmp_coefs <- rbind(tmp_coefs, tmp2$coef[,LETTERS[1:8]])
  }  
  effects[[i]] <- tmp_coefs
}  

file.name1 <- paste0("qtl2/lods_", number_of_parts, "_", which_part, ".rds")
saveRDS(lods, file=file.name1)
file.name2 <- paste0("qtl2/effects_", number_of_parts, "_", which_part, ".rds")
