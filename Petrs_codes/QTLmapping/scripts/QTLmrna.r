...
# parameters
addscan.dir <- "../addscan_mrna/"
intscan.dir.Sex <- "../intscan_mrna/Sex/"
intscan.dir.Age <- "../intscan_mrna/Age/"
...
output.file1 <- "../output/eQTLBestperGene.csv"
output.file2 <- "../output/eQTLAllAdditive.csv"
output.file3 <- "../output/eQTLAllInteractiveAge.csv"
output.file4 <- "../output/eQTLAllInteractiveSex.csv"

output <- annot.mrna[,c(1:5,9)]
output$AdditiveLOD <- output$AdditivePos <-  output$AdditiveChr <- NA
output$IntAgeLODFull <- output$IntAgeLODDiff <- output$IntAgePos <- 
  output$IntAgeChr <- NA
output$IntSexLODFull <- output$IntSexLODDiff <- output$IntSexPos <- 
  output$IntSexChr <- NA
...
for (i in 1:nrow(output)) {
  ...
  # additive scan
  dt <- data.frame(AdditiveChr=sapply(strsplit(rownames(fit0$lod),"_"), "[", 1),
                   AdditivePos=as.numeric(sapply(strsplit(rownames(fit0$lod),"_"), "[", 2)),
                   AdditiveLOD=fit0$lod[,1], stringsAsFactors=FALSE)
  ...
  # int. scan - Age
  stopifnot(rownames(fit0$lod) == rownames(fitAge$lod))
  dt <- data.frame(IntAgeChr=sapply(strsplit(rownames(fit0$lod),"_"), "[", 1),
                   IntAgePos=as.numeric(sapply(strsplit(rownames(fit0$lod),"_"), "[", 2)),
                   IntAgeLODDiff=fitAge$lod[,1] - fit0$lod[,1],
                   IntAgeLODFull = fitAge$lod[,1],
  ...
  # int. scan - Sex
  stopifnot(rownames(fit0$lod) == rownames(fitSex$lod))
  dt <- data.frame(IntSexChr=sapply(strsplit(rownames(fit0$lod),"_"), "[", 1),
                   IntSexPos=as.numeric(sapply(strsplit(rownames(fit0$lod),"_"), "[", 2)),
                   IntSexLODDiff=fitSex$lod[,1] - fit0$lod[,1],
                   IntSexLODFull = fitSex$lod[,1],
                   stringsAsFactors=FALSE)
